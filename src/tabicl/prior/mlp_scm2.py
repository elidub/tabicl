...
class MLPSCM(nn.Module):
    ...
    **kwargs : dict
    ...
    def forward(self):
        ...
        X, y, idx_X, idx_y = self.handle_outputs(causes, outputs)
        ...
        # Calculate adjacency matrix based on the indices used in this forward pass
        # adj_matrix = self._compute_adjacency(idx_X, idx_y)
    ...
    def handle_outputs(self, causes, outputs):
        ...
        if self.is_causal:
            ...
            if self.y_is_effect:
                # Indices for last layer outputs (converted to positive indices)
                total_dims = outputs_flat.shape[-1]
                indices_y = torch.arange(total_dims - self.num_outputs, total_dims, device=self.device)
            else:
    ...
    def _get_full_connectivity(self) -> torch.Tensor:
        ...
        """
        if self._cached_full_path_matrix is not None:
            return self._cached_full_path_matrix
        ...
        self._cached_full_path_matrix = path_matrix
        return path_matrix

    def _compute_adjacency(self, idx_X: Optional[torch.Tensor], idx_y: Optional[torch.Tensor]) -> torch.Tensor:
