...
def get_positions(width_layers: List[int], width_plot: float = 0.2) -> Dict[int, np.ndarray]:

    flatten_list = lambda l: [item for sublist in l for item in sublist]

    # ys = np.array([i / len(width_layers) for i, width_layer in enumerate(width_layers) for j in range(width_layer)])
    ys = [[y / len(width_layers)]*width_layer for y, width_layer in enumerate(width_layers)]
    xs = [np.arange(-width_plot * (width_layer - 1) / 2, width_plot * (width_layer - 1) / 2 + width_plot, width_plot).tolist() if width_layer % 2 == 0 else np.linspace(-width_plot * (width_layer - 1) / 2, width_plot * (width_layer - 1) / 2, width_layer).tolist() for width_layer in width_layers ]


    pos = {node_idx : np.array([y, x]) for node_idx, (x, y) in enumerate(zip(flatten_list(xs), flatten_list(ys)))}
    assert all([len(x) == len(y) == width_layer for x, y, width_layer in zip(xs, ys, width_layers)])
    return pos

def marginalize_nodes_array(adj_matrix: np.ndarray, indices_to_remove: np.ndarray | List[int]) -> np.ndarray:
    """
